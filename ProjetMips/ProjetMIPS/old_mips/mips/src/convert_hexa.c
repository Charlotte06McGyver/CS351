...
char tohexa(int value){//convertit la valeur décimale en hexadécimal
  char resultat;
  if(value == 10){
    resultat = 'a';
  }else if(value == 11){
...
int byte(int value){//Calcule le nombre de bits nécessaire pour coder notre décimal en binaire
  int i= 0;
  int a = 1;
  while(a == 1){
    if(pow(2,i)<= value){
...
void bit(int value, int size, char *tab){//Convertit un décimal en binaire sur seize bits
  int res = 0;
  int i = 0;
  int index = 0;
  int index_of_1 = 0;
  ...
  if(value>=0){
    while(value != 0){
      index = byte(value);//Nous donne le nombre de bits nécessaire pour coder notre nombre
