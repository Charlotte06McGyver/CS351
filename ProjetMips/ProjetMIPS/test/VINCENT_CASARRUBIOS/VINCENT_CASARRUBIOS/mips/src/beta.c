...
int nb_op_good(int nb_op_fonc, int tab[3]){// Teste si nous avons bien le bon nombre de paramètres dans notre fonction
    ...
    for(int i =0; i<3; i++){
        if(tab[i] != 65536 ){//Si une valeur dans le tableau est différente de 65536 alors ça veut dire qu'on a bien donné un argument
            nb++;
        }
    }
...
void concat_I(char *mot,int tab[3], int *tab_file, char *opcode){//Créer les instructions de type I
    ...
    mot = strcat(mot,opcode);// On concatène dans le bonne ordre nos registres avec l'opcode
    mot[6] = '\0';
    mot = strcat(mot,rs);
    mot[6+5] = '\0';
    mot = strcat(mot,rt);
...
void fonctions(char *commande,int mode,char *assemblage_sortie){
    ...
    if(in == 1){
        ...
        if(type(ligne) == 'R'){//teste si c'est un type R
            lecture_operandeR(commande,tab);// On lit les arguments qui nous sont donnés
            if(nb_op_good(nb_op,tab) == 1){// teste si on a bien le bon nombre d'arguments 
                ...
                fatal("Trop peu d'argument ou format incorrect: commande $a, $b, $c\n");//Erreur s'il n'y a pas assez d'arguments
            }
        }else if(type(ligne) == 'I'){//teste si c'est un type I
            lecture_operandeI(commande,tab);//On lit les argument qui nous sont donnés
            if(tab[2] != 65536){
            if((tab[0]<0) || (tab[1]<0) || (tab[0]>31) || (tab[1]>31)){// Teste si les valeurs des registres donnés ne sont pas trop grands/petits
                fatal("Insérer des registres entre 0 et 31");
            }
            if((tab[2]>65535) || (tab[2]< -32768 || (tab[2] == -48))){// teste si la valeur immédiate n'est pas trop grande ou trop petite
                fatal("Insérer une valeur imédiate entre -32768 et 65535");
            }
            }else{
                if((tab[0]<0) ||(tab[0]>31)){// Teste si les valeurs des registres donnés ne sont pas trop grands/petits
                fatal("Insérer des registres entre 0 et 31");
            }
                if((tab[1]>65535) || (tab[1]< -32768 || (tab[1] == -48))){// teste si la valeur immédiate n'est pas trop grande ou trop petite
                fatal("Insérer une valeur imédiate entre -32768 et 65535");
            }
            }
            if(nb_op_good(nb_op,tab) == 1){// teste si on a bien le bon nombre d'arguments
            ...
            if((tab[0]<67108864) || (tab[0]>=0)){// teste si on a bien le bon nombre d'arguments 
                ...
                fatal("Trop peu d'argument ou format incorrect: commande a\n");//Erreur s'il n'y a pas assez d'arguments
            }
